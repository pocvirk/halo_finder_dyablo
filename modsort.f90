...
Contains
  ...
  Subroutine tritas(tref,tid,tx,tv,n,tid4)
    ...
    Real   (kind=SP), Intent(inout),dimension(3,*) :: tx,tv     ! positions and velocities
    Integer(kind=PRI),Intent(inout),dimension(*)   :: tref, tid ! structure id and particle id
    Integer(kind=PRI4),Intent(inout),dimension(*)   :: tid4 ! particle id4 
    ...
    Call construction(tref,tid,tx,tv,size,n,tid4)
...
!!$    Print *,'Construction du tas terminee'
    ...
    Do i = n, 2, -1
       Call echanger(tref,tid,tx,tv,1,i,tid4)
       size = size - 1
       Call entasser(tref,tid,tx,tv,size,1,tid4)
    End Do

  End Subroutine tritas

  
  Subroutine construction(tref,tid,tx,tv,size,n,tid4)
    ...
    Real   (kind=SP), Intent(inout),dimension(3,*) :: tx,tv     ! positions and velocities
    Integer(kind=PRI),Intent(inout),dimension(*)   :: tref, tid ! structure id and particle id
    Integer(kind=PRI4),Intent(inout),dimension(*)   :: tid4 ! structure id and particle id                  
    ...
    size = n
    Do i = n/2, 1, -1
       Call entasser(tref,tid,tx,tv,size,i,tid4)
    End Do

  End Subroutine construction


  Recursive Subroutine entasser(tref,tid,tx,tv,size,i,tid4)
    ...
    If( max /= i ) Then
       Call echanger(tref,tid,tx,tv,i,max,tid4)
       Call entasser(tref,tid,tx,tv,size,max,tid4)
    End If

  End Subroutine entasser


  Subroutine echanger(tref,tid,tx,tv,i,j,tid4)
