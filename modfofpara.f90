...
!    Integer(kind=4), dimension(mynpart) :: lamas,pile,indl
    ...
    Integer(kind=PRI4), dimension(:), allocatable :: strAva4,strArr4,strBas4,strDro4,strGau4,strHau4,strRec4
    ...
    Integer(kind=PRI4), dimension(:), allocatable :: stf4, strSend4 ! structure ID array, and tmp array used for comm
    Integer(kind=PRI), dimension(:), allocatable :: idf, idSend  ! particle ID array and tmp array used for comm
    Integer(kind=PRI4), dimension(:), allocatable :: idf4, idSend4  ! particle ID array and tmp array used for comm
...
!    Integer(kind=PRI), dimension(mynpart) :: structure  ! structure ID for each particle in the process
    ...
    Integer(kind=PRI4), dimension(:), allocatable :: structure4
    ...
    Allocate(border(mynpart), lamas(mynpart), indl(mynpart), pile(mynpart), structure(mynpart),structure4(mynpart))
    ...
    If(nbh /= 0) Allocate (posHau(3,nbh), strHau(nbh), strHau4(nbh))
    If(nbb /= 0) Allocate (posBas(3,nbb), strBas(nbb), strBas4(nbb))
    If(nbr /= 0) Allocate (posArr(3,nbr), strArr(nbr), strArr4(nbr))
    If(nba /= 0) Allocate (posAva(3,nba), strAva(nba), strAva4(nba))
    If(nbd /= 0) Allocate (posDro(3,nbd), strDro(nbd), strDro4(nbd))
    If(nbg /= 0) Allocate (posGau(3,nbg), strGau(nbg), strGau4(nbg))
    ...
    raccordement : Do
       ...
       Allocate (posRec(3,nbrec),strRec(nbrec),strRec4(nbrec))
       ...
       Call Mpi_ISend(strBas4,nbb,  MPI_PRI4,sendID,sendID,MPICube,mpireqs1,mpierr)
       Call Mpi_IRecv(strRec4,nbrec,MPI_PRI4,recvID,procID,MPICube,mpireqr1,mpierr)
       Call Mpi_Wait(mpireqs1,mpistat,mpierr)
       Call Mpi_Wait(mpireqr1,mpistat,mpierr)
          ...
          sonbas : Do j=1, nbrec
             ...
             raccordh : If(d <= r2 .And. strRec(j)< strHau(i) ) Then
                ...
                renumallh : Do k=1,mynpart
                   If(structure(k) == strHau(i)) Then
                   structure(k) = strRec(j)
                   structure4(k)=strRec4(j)
                   End If
       ...
       Deallocate(posRec,strRec,strRec4)
    ...
    Allocate(idf4(myNpartW),STAT=allStat)
    ...
    procMasse : Do iproc = 1,procNB-1
       ...
       If(nbsend /= 0) Then
          Allocate(strSend(nbsend),posSend(3,nbsend),velSend(3,nbsend),idSend(nbsend),idSend4(nbsend))
          ...
          Call Mpi_Isend(idSend4, nbsend,  MPI_PRI4, sendID,5,MPICube,mpireqs5,mpierr)
       ...
       If(nbrec /= 0) Then
          ...
          Call Mpi_IRecv(idf4(recvpoint), nbrec,  MPI_PRI4, recvID,5,MPICube,mpireqr5,mpierr)
       ...
       If(nbsend/=0) Then
          ...
          Deallocate(strSend,posSend,idSend,idSend4,velSend)
       End If
       If(nbrec/=0) Then
          Call Mpi_Wait(mpireqr1,mpistat,mpierr)
    ...
    !! ************ ADD idf4 to tritas **************
    Call tritas(stf,idf,xf,vf,myNpartW,idf4)
    ...
    Call outputstruct(myNpartW,NstrW,nbs,idf,xf,vf,massamas,idf4)
